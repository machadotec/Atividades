...
function tmultifield_start( name, fields, mandatories, width, height ) {
    if (typeof multifields == 'undefined') {
        multifields = new Object;
    }
    
    multifields[name] = new MultiField(name+'mfTable', width, height );
    multifields[name].formFieldsAlias = fields;
    multifields[name].formFieldsName = Array();
    multifields[name].formPostFields = Array();
    
    for (key in fields) {
        col = fields[key];
        multifields[name].formPostFields[col] = 1;
        multifields[name].formFieldsName.push(name+'_'+col);
    }
    
    multifields[name].formFieldsMandatory = mandatories;
    multifields[name].mandatoryMessage = 'The field ^1 is required';
    multifields[name].storeButton  = document.getElementsByName(name+'btnStore')[0];
    multifields[name].deleteButton = document.getElementsByName(name+'btnDelete')[0];
    multifields[name].cancelButton = document.getElementsByName(name+'btnCancel')[0];
    multifields[name].inputResult  = document.getElementsByName(name)[0];
}
